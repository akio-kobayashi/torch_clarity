...
def ear_model(
    ...
    Arguments:
        reference (Tensor): reference signal: should be adjusted to 65 dB SPL
            (itype=0 or 1) or to 65 dB SPL plus NAL-R gain (itype=2)
        reference_freq (int): sampling rate for the reference signal, Hz
        processed (Tensor): processed signal (e.g. hearing-aid output) includes
            HA gain
        processed_freq (int): sampling rate for the processed signal, Hz
        hearing_loss (Tensor): audiogram giving the hearing loss in dB at 6
    ...
    '''
    # Use torchaudio.functional.resample instead of resample_24khz function
    ...
    freq_sample = 24000
    reference_24hz = torch.squeeze(F.resample(torch.unsqueeze(reference, dim=0), reference_freq))
    processed_24hz = torch.squeeze(F.resample(torch.unsqueeze(processed, dim=0), processed_freq))
    ...
    reference_db = torch.zeros((nchan, nsamp), device=reference.device)
    processed_db = torch.zeros((nchan, nsamp), devcie=reference.device)

    # Reference and processed average spectral values
    reference_average = torch.zeros(nchan, device=reference.device)
    processed_average = torch.zeros(nchan, device=reference.device)
    reference_control_average = torch.zeros(nchan, device=reference.device)
    processed_control_average = torch.zeros(nchan, device=reference.device)

    # Filter bandwidths adjusted for intensity
    reference_bandwidth = torch.zeros(nchan, device=reference.device)
    processed_bandwidth = torch.zeros(nchan, device=reference.device)

    reference_b = torch.zeros((nchan, nsamp), device=reference.device)
    processed_b = torch.zeros((nchan, nsamp), device=reference.device)
    ...
    for n in range(nchan):
        ...
        reference_control_average[n] = torch.sqrt(torch.mean(reference_control**2))
        processed_control_average[n] = torch.sqrt(torch.mean(processed_control**2))
...
def input_align(reference: Tensor, processed: Tensor) -> tuple[Tensor, Tensor]:
    """
    ...
    Arguments:
    reference (Tensor): input reference sequence
    processed (Tensor): hearing-aid output sequence

    Returns:
    reference (Tensor): pruned and shifted reference
    processed (Tensor): pruned and shifted hearing-aid output
    ...
    # Compute lags for alignment by np.correlate
    np_reference = reference.cpu().detach().numpy()
    np_processed = processed.cpu().detach().numpy()
    # Determine the delay of the output relative to the reference
    reference_processed_correlation = correlate(
        np_reference[:min_sample_length] - np.mean(np_reference[:min_sample_length]),
        np_processed[:min_sample_length] - np.mean(np_processed[:min_sample_length]),
        "full",
    ...
    if delay > 0:
        ...
        processed = torch.cat((torch.zeros(-delay), processed[: processed_n + delay]))
...
def middle_ear(reference: Tensor, freq_sample: float) -> Tensor:
    """
    ...
    y = F.lfilter(torch.tensor(butterworth_low_pass, device=reference.device), 
                  torch.tensor(low_pass, device=reference.device), 
                  reference)
    ...
    y = F.lfilter(torch.tensor(butterworth_high_pass, device=reference.device),
                  torch.tensor(high_pass, device=reference.device),
                  y)
    return y
...
def gammatone_basilar_membrane(
    ...
    sincf, coscf = gammatone_bandwidth_demodulation(
        npts, tpt, center_freq, torch.zeros(npts,device=reference.device), torch.zeros(npts, device=reference.device)
    )

    # Filter the real and imaginary parts of the signal
    ureal = F.lfilter([1, a_1, a_5], [1, -a_1, -a_2, -a_3, -a_4], x * coscf)
    uimag = F.lfilter([1, a_1, a_5], [1, -a_1, -a_2, -a_3, -a_4], x * sincf)
    ...
    # Filter the real and imaginary parts of the signal
    ureal = F.lfilter([1, a_1, a_5], [1, -a_1, -a_2, -a_3, -a_4], y * coscf)
    uimag = F.lfilter([1, a_1, a_5], [1, -a_1, -a_2, -a_3, -a_4], y * sincf)
...
def env_compress_basilar_membrane(
    envsig: Tensor,
    bm: Tensor,  # pylint: disable=invalid-name
    control: Tensor,
    attn_ohc: float,
    threshold_low: float,
    ...
    logenv = torch.maximum(control, small)
    logenv = level1 + 20 * torch.log10(logenv)
    logenv = torch.minimum(
        logenv, threshold_high
    )  # Clip signal levels above the upper threshold
    logenv = torch.maximum(logenv, threshold_low)  # Clip signal at the lower threshold
    ...
    gain = F.lfilter(torch.from_numpy(b).to(gain.device), 
                     torch.from_numpy(a).to(gain.device), 
                     gain)
...
def envelope_align(
    ...
    ref_out_correlation = correlate(np_reference, np_output, "full")
    location = np.argmax(
    ...
    if delay > 0:
        # Output delayed relative to the reference
        return torch.cat((output[delay:npts], torch.zeros(delay, device=output.device)))
    return torch.cat((torch.zeros(-delay, device=output.device), output[: npts + delay]))


def envelope_sl(
    reference: Tensor,
    basilar_membrane: Tensor,
    ...
    _reference = level1 - attenuated_ihc + 20 * torch.log10(reference + small)
    _reference = torch.maximum(_reference, 0)
...
def inner_hair_cell_adaptation(
    reference_db: Tensor, 
    reference_basilar_membrane: Tensor, 
    delta, 
    freq_sample
):
    """
    ...
    gain = torch.ones_like(
        reference_db,
        device=reference_db.device
    )  # Gain vector to apply to the BM motion, default is 1
    output_db = torch.zeros_like(reference_db, device=reference_db.device)
    v_1 = 0
    v_2 = 0
    small = 1e-30
    ...
    output_db = torch.maximum(output_db, 0)
...
def basilar_membrane_add_noise(
    ...
    noise = gain * torch.normal(
        mean=torch.zeros_like(reference, device=reference.device),
        std=torch.ones_like(reference, device=reference.device)
    )
...
def group_delay_compensate(
    ...
    a = torch.exp(-tpt_bandwidth)
    
    # compute by numpy
    a_1 = (4.0 * a).cpu().detach().numpy()
    a_2 = (-6.0 * a * a).cpu().detach().numpy()
    a_3 = (4.0 * a * a * a).cpu().detach().numpy()
    a_4 = (-a * a * a * a).cpu().detach().numpy()
    a_5 = (4.0 * a * a).cpu().detach().numpy()
    ...
    # ndarray to tensor
    _group_delay = torch.from_numpy(_group_delay).to(reference.device)
    ...
    processed = torch.zeros(reference.shape, device=reference.device)
    for n in range(nchan):
        ref = reference[n]
        npts = len(ref)
        processed[n] = torch.cat((torch.zeros(correct[n], device=reference.device), ref[: npts - correct[n]]))
...
def convert_rms_to_sl(
    reference: Tensor,
    control: Tensor,
    attenuated_ohc: Tensor | float,
    threshold_low: Tensor | int,
    compression_ratio: Tensor | int,
    attenuated_ihc: Tensor | float,
    level1: float,
    threshold_high: int = 100,
    small: float = 1e-30,
) -> Tensor:
    ...
    small = 1e-30
    control_db_spl = torch.maximum(control, small)
    control_db_spl = level1 + 20 * torch.log10(control_db_spl)
    control_db_spl = torch.minimum(control_db_spl, threshold_high)
    control_db_spl = torch.maximum(control_db_spl, threshold_low)
    ...
    control_db_spl = torch.maximum(reference, small)
    control_db_spl = level1 + 20 * torch.log10(control_db_spl)
    control_db_spl = torch.maximum(control_db_spl, 0)
    reference_db = control_db_spl + gain - attenuated_ihc
    reference_db = torch.maximum(reference_db, 0)

    return reference_db


def env_smooth(envelopes: Tensor, 
               segment_size: int, 
               sample_rate: float) -> Tensor:
    """
    ...
    Arguments:
        envelopes (Tensor): matrix of envelopes in each of the auditory bands
    ...
    window = torch.from_numpy(np.hanning(n_samples)).to(device)  # Raised cosine von Hann window
    ...
    smooth = torch.zeros((n_channels, nseg), device=device)
    ...
    for k in range(n_channels):
        ...
        nstart = 0
        smooth[k, 0] = torch.sum(r[nstart:nhalf] * torch.transpose(torch.conj_physical(halfwindow))) / halfsum

        # Loop over the remaining full segments, 50% overlap
        for n in range(1, nseg - 1):
            nstart = int(nstart + nhalf)
            nstop = int(nstart + n_samples)
            smooth[k, n] = sum(r[nstart:nstop] * torch.transpose(torch.conj_physical(window))) / wsum

        # The last (half) windowed segment
        nstart = nstart + nhalf
        nstop = nstart + nhalf
        smooth[k, nseg - 1] = (
            torch.sum(r[nstart:nstop] * torch.transpose(torch.conj_physical(window[:nhalf]))) / halfsum
        )
...
def mel_cepstrum_correlation(
    ...
    freq = torch.from_numpy(np.arange(nbasis)).to(device)
    k = torch.from_numpy(np.arange(nbands)).to(device)
    basis = torch.cos(torch.outer(k, freq) * np.pi / float(nbands - 1))
    mel_cepstral = basis / torch.norm(basis, axis=0)

    # Find the segments that lie sufficiently above the quiescent rate
    reference_linear = 10 ** (
        reference / 20
    )  # Convert envelope dB to linear (specific loudness)
    xsum = torch.sum(reference_linear, 0) / nbands  # Proportional to loudness in sones
    xsum = 20 * torch.log10(xsum)  # Convert back to dB (loudness in phons)
    index = torch.where(xsum > threshold)[0]  # Identify those segments above threshold
    ...
    individual_cepstral_correlations = torch.zeros(nbasis, device=device)
    if nsamp <= 1:
        logger.warning(
    ...
    ref = ref + addnoise * torch.normal(torch.zeros_like(ref.shape).to(device),
                                        torch.ones_like(ref.shape).to(device))
    proc = proc + addnoise * torch.normal(torch.zeros_like(proc.shape).to(device),
                                          torch.ones_like(proc.shape).to(device))

    # Compute the mel cepstrum coefficients using only those segments
    # above threshold

    reference_cep = torch.dot(mel_cepstral.T, ref)
    processed_cep = torch.dot(mel_cepstral.T, proc)
    ...
    reference_cep -= torch.mean(reference_cep, axis=1, keepdims=True)
    processed_cep -= torch.mean(processed_cep, axis=1, keepdims=True)
    ...
    small = 1.0e-30
    xsum = torch.sum(reference_cep**2, axis=1)
    ysum = torch.sum(processed_cep**2, axis=1)
    mask = torch.logical_or((xsum < small) , (ysum < small))
    individual_cepstral_correlations = torch.zeros(nbasis, device=device)
    individual_cepstral_correlations[torch.logical_not(mask)] = torch.abs(
        torch.sum(reference_cep[torch.logical_not(mask)] * processed_cep[torch.logical_not(mask)], axis=1)
        / torch.sqrt(xsum[torch.logical_not(mask)] * ysum[torch.logical_not(mask)])
    )

    # Figure of merit is the average of the cepstral correlations, ignoring
    # the first (average spectrum level).
    average_cepstral_correlation = torch.sum(
...
def spectrum_diff(
    reference_sl: Tensor, processed_sl: Tensor
    ...
    reference_sum = torch.sum(reference_linear_magnitude)
    reference_linear_magnitude /= (
        reference_sum  # Loudness sum = 1 (arbitrary amplitude, proportional to sones)
    )
    processed_sum = torch.sum(processed_linear_magnitude)
    processed_linear_magnitude /= processed_sum

    # Compute the spectrum difference
    dloud = torch.zeros(3, device=device)
    diff_spectrum = (
        reference_linear_magnitude - processed_linear_magnitude
    )  # Difference in specific loudness in each band
    dloud[0] = torch.sum(torch.abs(diff_spectrum))
    dloud[1] = nbands * torch.std(diff_spectrum)  # Biased std: second moment
    dloud[2] = torch.max(torch.abs(diff_spectrum))

    # Compute the normalized spectrum difference
    dnorm = torch.zeros(3, device=device)
    diff_normalised_spectrum = (
        reference_linear_magnitude - processed_linear_magnitude
    ) / (
        reference_linear_magnitude + processed_linear_magnitude
    )  # Relative difference in specific loudness
    dnorm[0] = torch.sum(torch.abs(diff_normalised_spectrum))
    dnorm[1] = nbands * torch.std(diff_normalised_spectrum)
    dnorm[2] = torch.max(torch.abs(diff_normalised_spectrum))

    # Compute the slope difference
    dslope = torch.zeros(3, device=device)
    reference_slope = (
    ...
    dslope[0] = torch.sum(torch.abs(diff_slope))
    dslope[1] = nbands * torch.std(diff_slope)
    dslope[2] = torch.max(torch.abs(diff_slope))
...
def bm_covary(
    reference_basilar_membrane: Tensor,
    processed_basilar_membrane: Tensor,
    segment_size: int,
    sample_rate: float,
) -> tuple[Tensor, Tensor, Tensor]:
    """
    ...
    device = reference_basilar_membrane.device
    ...
    reference_mean_square = torch.zeros((nchan, nseg), device=device)
    processed_mean_square = torch.zeros((nchan, nseg), device=device)
    signal_cross_covariance = torch.zeros((nchan, nseg), device=device)

    # convert to Tensor
    half_window = torch.from_numpy(half_window).to(device)
    half_corr = torch.from_numpy(half_corr).to(device)
    window = torch.from_numpy(window).to(device)
    win_sum2 = torch.from_numpy(win_sum2).to(device)
    win_corr = torch.from_numpy(win_corr).to(device)
    ...
    for k in range(nchan):
        ...
        reference_seg = reference_seg - torch.mean(reference_seg)  # Make 0-mean
        processed_seg = processed_seg - torch.mean(processed_seg)

        # Normalize signal MS value by the window
        ref_mean_square = torch.sum(reference_seg**2) * halfsum2

        proc_mean_squared = torch.sum(processed_seg**2) * halfsum2
        padding = len(processed_seg)//2
        correlation = torch.functional.conv1d(rearrange(reference_seg, '(b c t)->b c t', b=1, c=1), rearrange(processed_seg, '(b c t)->b c t', b=1, c=1), padding=padding)
        assert correlation.shape[-1] >= len(reference_seg)
        correlation = rearrange(correlation, 'b c t -> (b c t)')[:len(reference_seg)]        
        #correlation = correlate(reference_seg, processed_seg, "full")
        correlation = correlation[
            int(len(reference_seg) - 1 - maxlag) : int(maxlag + len(reference_seg))
        ]
        unbiased_cross_correlation = torch.max(torch.abs(correlation * half_corr))
        if (ref_mean_square > small) and (proc_mean_squared > small):
            # Normalize cross-covariance
            signal_cross_covariance[k, 0] = unbiased_cross_correlation / torch.sqrt(
        ...
        for n in range(1, nseg - 1):
            ...
            # Normalize signal MS value by the window
            ref_mean_square = torch.sum(reference_seg**2) * win_sum2
            proc_mean_squared = torch.sum(processed_seg**2) * win_sum2
            padding = len(processed_seg)//2
            correlation = torch.functional.conv1d(rearrange(reference_seg, '(b c t)->b c t', b=1, c=1), rearrange(processed_seg, '(b c t)->b c t', b=1, c=1) )
            correlation = rearrange(correlation,'b c t -> (b c t)')
            #correlation = correlate(reference_seg, processed_seg, "full")
            correlation = correlation[
                int(len(reference_seg) - 1 - maxlag) : int(maxlag + len(reference_seg))
            ]
            unbiased_cross_correlation = torch.max(torch.abs(correlation * win_corr))
            if (ref_mean_square > small) and (proc_mean_squared > small):
                # Normalize cross-covariance
                signal_cross_covariance[k, n] = unbiased_cross_correlation / torch.sqrt(
                    ref_mean_square * proc_mean_squared
                )
            else:
        ...
        if (ref_mean_square > small) and (proc_mean_squared > small):
            ...
            signal_cross_covariance[k, nseg - 1] = unbiased_cross_correlation / torch.sqrt(
                ref_mean_square * proc_mean_squared
            )
        else:
    ...
    signal_cross_covariance = torch.clip(signal_cross_covariance, 0, 1)
...
def ave_covary2(
    signal_cross_covariance: Tensor,
    reference_signal_mean_square: Tensor,
    threshold_db: float,
    lp_filter_order: Tensor | None = None,
    freq_cutoff: Tensor | None = None,
) -> tuple[float, Tensor]:
    """
    ...
    device = signal_cross_covariance.device
    ...
    signal_rms = torch.sqrt(reference_signal_mean_square)
    # Linear amplitude (specific loudness)
    signal_linear_amplitude = 10 ** (signal_rms / 20)
    # Intensity averaged over frequency bands
    reference_mean = torch.sum(signal_linear_amplitude, 0) / n_channels
    # Convert back to dB (loudness in phons)
    reference_mean = 20 * torch.log10(reference_mean)
    # Identify those segments above threshold
    index = torch.argwhere(reference_mean > threshold_db).T
    if index.size != 1:
        index = index.squeeze()
    ...
    if nseg <= 1:
        ...
        # syncov = 0
        ihc_sync_covariance = torch.zeros(6).to(device)
    ...
    weight = torch.zeros((n_channels, nseg)).to(device)  # No IHC synchronization roll-off
    ...
    wsync = torch.zeros((6, n_channels, nseg)).to(device)
    mask = signal_rms > threshold_db
    fsync3d = torch.from_numpy(np.repeat(fsync[..., None], nseg, axis=2)).to(device)
    wsync[:, mask] = fsync3d[:, mask]

    # Sum the weighted covariance values
    # Sum of weighted time-freq tiles
    csum = torch.sum(torch.sum(weight * signal_cross_covariance))

    wsum = torch.sum(torch.sum(weight))  # Total number of tiles above threshold

    tiles_above_threshold = torch.zeros(6).to(device)

    # Sum of weighted time-freq tiles
    sum_weighted_time_freq = torch.sum(wsync * signal_cross_covariance, axis=(1, 2))

    tiles_above_threshold = torch.sum(wsync, axis=(1, 2))
