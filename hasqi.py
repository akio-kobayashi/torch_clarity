...
def hasqi_v2(
    ...
    Arguments:
        reference (Tensor): Clear input reference speech signal with no noise or
        ...
        processed (Tensor): Output signal with noise, distortion, HA gain, and/or
            processing.
    ...
    d_loud = torch.maximum(torch.minimum(d_loud, 1.0), 0.0)

    # Dslope:std
    d_slope = d_slope_stats[1]  # Slope difference std
    d_slope = 1.0 - d_slope
    d_slope = torch.maximum(torch.minimum(d_slope, 1.0), 0.0)
